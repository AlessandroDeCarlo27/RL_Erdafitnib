library(ggplot2)
#non c'è R studio, setto wd a mano

#mettere modello e affini nella cartella /local/tmp/ del cluster. Local è
# una partizione con SSD --> maggiore velocità di lettura e scrittura


setwd("/home/magni/aledeca/ERDAF2")
...
for(j in 1:length(vett_niter)){
  ...
  inTime<-Sys.time()
  for(iter in 1:niter){
    dataset<-read.csv("/local/tmp/ERDAF/DATASET_50sog.csv")
    ...
    dataset$C<-as.character(dataset$C)
      for(k in 1:nmonth){
        if(k==1){
          ...
          dati<-read.delim("/local/tmp/ERDAF/sdtab2",sep="",skip=1)
	...
	for(i in 1:nsog){
        ...
        write.csv(dataset, "/local/tmp/ERDAF/DATASET_50sog.csv", quote=F, row.names=F)
        system2(command="/cm/shared/apps/NONMEM/run/nmfe74",
                args="mod_fin_sim.mod mod_fin.lst -licfile=nonmem.lic -rundir=/local/tmp/ERDAF/")
